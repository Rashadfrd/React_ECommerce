...
const Navbar = () => {
  ...
  return (
    <header className={location.pathname.length > 1 ? `${classes.header} ${classes.headerSticky}`: `${classes.header} ${classes.headerAbsolute}`}>
        <div>
            <Link to={'/'}><img src={img} width={'150'} alt="" /></Link>
        </div>
        <nav className={classes.nav}>
            {links.map((link)=>{
                const{id,text,url} = link;
                return <NavLink key={id} to={url} className={location.pathname === url ? `${classes.navItemSecond}  ${classes.active}` : `${classes.navItemSecond}`}>{text}</NavLink>
            })}
            {
        ...
        <div className={classes.menuBtn} onClick={sidebarHandler}><span><AiOutlineMenu color='black' size={'30px'} /></span></div>
    </header>
  )
}
